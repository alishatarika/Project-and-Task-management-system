...
class RegisterSchema(BaseModel):
    ...
    def email_valid(cls, v):
        v = v.lower().strip()
        if not v:                       raise ValueError("Email cannot be empty")
        if ".." in v:                   raise ValueError("Email cannot contain consecutive dots")
        local, _ = v.split("@")
        if local.startswith(".") or local.endswith("."): raise ValueError("Email local part cannot start/end with dot")
        return v
    ...
    def passwords_match(cls, v, values):
        if not v:                                   raise ValueError("Confirm Password cannot be empty")
